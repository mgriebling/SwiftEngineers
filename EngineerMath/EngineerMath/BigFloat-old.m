...
//
void BF_ClearValuesArray(unsigned long *values, unsigned int multiple)
{
	int i;
...
//
BOOL BF_ArrayIsNonZero(unsigned long *values, unsigned int multiple)
{
	int i;
...
//
void BF_CopyValues(unsigned long *source, unsigned long *copyArray)
{
	int	i;
...
//
void BF_AssignValues(unsigned long *destination, unsigned long *copyArray)
{
	int	i;
...
//
void BF_AddToMantissa(unsigned long *values, unsigned long digit, unsigned long limit, unsigned int multiple)
{
	int i;
...
//
void BF_AppendDigitToMantissa(unsigned long *values, unsigned long digit, unsigned short radix, unsigned long limit, unsigned int multiple)
{
	int i;
...
//
signed long BF_RemoveDigitFromMantissa(unsigned long *values, unsigned short radix, unsigned long limit, unsigned int multiple)
{
...
//
signed long BF_RemoveDigitFromMantissaAndFlagEmpty(unsigned long *values, unsigned short radix, unsigned long limit, unsigned int multiple, BOOL *isEmpty)
{
	// Truncate a digit by dividing through by the bf_radix
	unsigned long	carryBits = 0;
	int					i;
...
//
long BF_NumDigitsInArray(unsigned long *values, unsigned short radix, unsigned long precision)
{
	int digitsInNumber;
	int valueNumber;
	int digitNumber;
...
- (void)setElements:(unsigned short)radix negative:(BOOL)isNegative exp:(signed short)exponent valid:(BOOL)isValid userPoint:(unsigned short)userPoint
{
	...
	bf_value_limit = pow(radix, bf_value_precision);
...
- (id)initWithDouble:(double)newValue radix:(unsigned short)newRadix
{
	...
	if (doubleExponent < 0)
		newExponent = floor(doubleExponent);
	else
		newExponent = ceil(doubleExponent);
...
- (id)initWithCoder:(NSCoder *)coder
{
	...
	bf_exponent_precision = [coder decodeInt64ForKey:@"BFExponentPrecision"];
...
- (void)encodeWithCoder:(NSCoder *)coder
{
	...
	[coder encodeInt64:bf_exponent_precision forKey:@"BFExponentPrecision"];
...
- (void)convertToRadix: (unsigned short)newRadix
{
	...
	elements.bf_value_limit = pow(newRadix, elements.bf_value_precision);
